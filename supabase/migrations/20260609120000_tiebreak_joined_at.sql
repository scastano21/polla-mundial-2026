-- Desempate: puntos → exactos → quien se unió primero a la polla.
...
BEGIN
  WITH ranked AS (
    SELECT id,
           ROW_NUMBER() OVER (
             ORDER BY total_points DESC, exact_scores DESC, joined_at ASC, user_id ASC
           ) AS new_rank
...
AS $$
  ...
  ORDER BY pm.total_points DESC, pm.exact_scores DESC, pm.joined_at ASC, pm.user_id ASC;
$$;

-- Recalcular posiciones en todas las pollas (ejecutar una vez tras migrar):
